...
鲸书中讲代码删除是“冗余代码删除 / 公共子表达式消除 / 循环不变代码外提”等, 借助数据流分析驱动的优化, 而这里的 DCEPass 是更简单的删除所有 “无人使用 + 无副作用” 的指令, 不需要复杂的数据流方程计算. 无副作用不用说就是上边的一堆判断, 那无人使用是怎么确定的呢, 这是 LLVM IR 的 use-def 链机制实时维护的. 简单来说就是在定义或者使用变量时维护一个 list, 这个并不在这里细讲.
